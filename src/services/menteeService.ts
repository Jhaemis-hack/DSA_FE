...
export const profileData = async function () {
  try {
    const res: ResponseInterface = await Axios.get(menteeRequest.myProfile);
...
export const updateUserProfile = async function (payload: any) {
  try {
...
export const fetchActiveMentors = async function () {
  try {
    const res: ResponseInterface = await Axios.get(menteeRequest.fetchMentors);
    if (res.data.status_code < 400) {
      // Success(res.data.message);
      return res.data;
    }
...
export const fetchAllMentors = async function () {
  try {
    const res: ResponseInterface = await Axios.get(
      menteeRequest.fetchAllMentors
    );
    if (res.data.status_code < 400) {
      // Success(res.data.message);
      return res.data;
...
export const BookASession = async function (
  mentorId: string,
  payload: { date: string }
) {
  try {
...
export const viewAvailableSessions = async function () {
  try {
    const res: ResponseInterface = await Axios.get(menteeRequest.viewSessions);
    if (res.data.status_code < 400) {
      Success(res.data.message);
      return res.data;
    }
